...
def create_tiff(nb_channels, new_tiff_name, width, height, datatype, data_array, geotransformation, projection):
    ...
    if nb_channels == 1:
        if len(data_array.shape) == 2:
            ...
            dst_ds.GetRasterBand(1).WriteArray(data_array[0])
    else:
...
def encode_image(encoder, loader, batch_size, nb_features, gpu, H, W, geo, proj, name_results, path_results):
    encoder.eval()
    new_coordinates = np.empty((0, nb_features), float)
    for batch_idx, (data, _, index) in enumerate(loader):
        if gpu:
            data = data.cuda()
            index = index.cuda()
